...
class TeekoPlayer:  
    ...
    def heuristic_gv(self, state, piece):
        ...
        # horizontal
        my_max = opp_max = my_cnt = opp_cnt = 0        
        ...
        if my_max == opp_max:
            ...
            return my_max/6, state # if mine is longer than opponent, return positive float

        return (-1) * opp_max/6, state # if opponent is longer than mine, return negative float
    ...
    def make_move(self, state):
        ...
        if not drop_phase:
            ...
            move = []
            value, b_state = self.Max_Value(state, 0)            
            arr1 = np.array(state) == np.array(b_state)
